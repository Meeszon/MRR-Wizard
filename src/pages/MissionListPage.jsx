...
import { ArrowLeft, Play, Pencil, Plus, FolderOpen, Locate, Clock, Battery } from 'lucide-react'
  ...
  return (
    <div className="w-full h-full flex flex-col bg-bg-secondary">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-4 flex flex-col gap-2.5 min-h-0">
        <ActiveList
          missions={missions}
          onEdit={(m) => handleEdit(m)}
          onStart={(m) => handleStart(m)}
...
function ActiveList({ missions, onEdit, onStart, onNew }) {
  if (missions.length === 0) {
    return (
      <div className="flex-1 flex flex-col gap-2.5">
        <NewMissionCard onNew={onNew} />
        <div className="flex-1 flex flex-col items-center justify-center gap-3">
          ...
          <p style={{ fontSize: '13px', color: '#888' }}>No missions created yet</p>
        </div>
      </div>
    )
  }
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button
                ...
                style={{ padding: '7px 10px', fontSize: '12px', fontWeight: 600, color: '#5A5A5A' }}
              >
